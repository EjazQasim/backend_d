...
export default {
     ...
     */
		registerUser: async (
			parent,
			{ name, email, password, records },
			context
		) => {
			console.log('asdfasdfasdfasdfas');
			if (!email || !password || !records) {
        ...
        await context.di.model.Users.find().estimatedDocumentCount();
			...
			const isAnEmailAlreadyRegistered =
        await context.di.model.Users.findOne({ email }).lean();

			if (isAnEmailAlreadyRegistered) {
				throw new UserInputError('Data provided is not valid');
			}
			const user = new context.di.model.Users({
				name,
				email,
				password,
				records,
			...
			const savedUser = await context.di.model.Users.findOne({
				email,
			}).lean();
		...
		authUser: async (parent, { email, password }, context) => {
			...
			if (!user) {
				throw new UserInputError(
					'User not found or login not allowed'
				);
			}
