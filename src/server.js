...
if (environmentVariablesConfig.formatConnection === 'DNSseedlist') {
  console.log(environmentVariablesConfig.mongoDNSseedlist);
...
db.once('open', () => {
  if (
    environmentVariablesConfig.environment !== ENVIRONMENT.DEVELOPMENT
  ) {
    logger.info(
  ...
  const server = new ApolloServer({
    ...
    introspection:
      ...
      ENVIRONMENT.PRODUCTION
        ? false
        : true, // Set to "true" only in development mode
    plugins:
      environmentVariablesConfig.environment ===
      ENVIRONMENT.PRODUCTION
        ? [ApolloServerPluginLandingPageDisabled()]
        : [
            requestDevLogger,
            ApolloServerPluginLandingPageGraphQLPlayground(),
          ], // Log all querys and their responses. Show playground (do not use in production)
    formatError(error) {
